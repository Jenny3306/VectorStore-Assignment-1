...
public:
    ...
    T removeAt(int index); // trả về gtri của ptu bị xóa
    bool empty() const;
    int size() const;
    void clear();
    T& get(int index); // mặc dù là trả về gtri của ptu tại index nhưng đây là trả về tham chiếu => T&: có thể sửa gtri ptu thông qua tham chiếu này
...
public:
    struct VectorRecord
    {
        int id; // index của ptu trong array
        string rawText;
        int rawLength;
...
private:
    ...
    int dimension;
    int count; // số ptu vector hiện có
